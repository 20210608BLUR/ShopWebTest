...
namespace ShopWebsite.Areas.Controllers
{
    ...
    public class NewsController : GenericController
    {
        ...
        public NewsController(ShopWebsiteContext context, IWebHostEnvironment hostingEnvironment) : base(context)
        {
            ...
            _newsRepository = new NewsRepository(context, hostingEnvironment);
        }
        ...
        public async Task<IActionResult> Index(NewsCreateViewModel newsViewModel)
        {            
            GetMenu();
            _newsRepository.GetMenuByWeb();

            //_newsRepository.tt();

            List<NewsIndexViewModel> viewModel = new List<NewsIndexViewModel>();

            // 取得新聞列表的 ViewModel
            viewModel = _newsRepository.GetList();
